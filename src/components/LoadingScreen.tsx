...
export default function LoadingScreen({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    // On mobile devices (<768px), bypass preloader entirely so top is 100% visible with 0 freeze!
    ...
    // On desktop, check if already seen in session
    try {
    ...
    // Desktop 1.5s smooth cyber counter
  ...
  // If not mounted (SSR), or if finished/mobile -> clean unmount immediately
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[120] hidden md:flex flex-col justify-between p-6 sm:p-10 md:p-14 bg-[#06070a] select-none overflow-hidden cursor-pointer transition-opacity duration-300"
    >
