...
export default function BusinessCardPage() {
  ...
  const handleSaveContact = () => {
    try {
      confetti({
        ...
        spread: 60,
        origin: { y: 0.7 },
        colors: ["#ffffff", "#e2e8f0", "#94a3b8", "#f59e0b", "#38bdf8"],
      });
    } catch (e) {
      // Ignore
    }
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: "Subhash Ketagoda - Digital Card",
  ...
  return (
    <div className="min-h-screen w-full bg-[#050608] text-white selection:bg-white selection:text-black flex flex-col justify-between relative overflow-x-hidden antialiased">
      {/* Apple Ambient Soft Vignette Glow */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-gradient-to-b from-white/[0.05] via-white/[0.015] to-transparent rounded-full blur-[140px]" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] bg-sky-500/[0.03] rounded-full blur-[160px]" />
      </div>
      ...
      <header className="relative z-10 w-full max-w-md mx-auto px-4 sm:px-6 pt-5 sm:pt-7 flex items-center justify-between">
        <Link
          href="/card"
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-white/70 hover:text-white text-xs font-sans transition-all active:scale-95 backdrop-blur-md"
        >
      ...
      <main className="relative z-10 w-full max-w-[340px] xs:max-w-[370px] sm:max-w-[400px] mx-auto px-3 xs:px-4 py-4 sm:py-6 flex flex-col items-center">
        ...
        <div className="text-center mb-4 sm:mb-5">
          <span className="text-[11px] xs:text-xs font-sans text-white/50 tracking-wide font-medium">
            Digital Pass &bull; Contact Card
          </span>
        </div>

        {/* =========================================================================
            APPLE MINIMALIST LUXURY BUSINESS CARD (3D TILT & FLIP)
           ========================================================================= */}
        <div
          ref={cardRef}
        ...
        >
          ...
          >
            {/* =========================================================================
                FRONT SIDE: APPLE SPACE-BLACK TITANIUM PASS
               ========================================================================= */}
            <div className="backface-hidden w-full h-full min-h-[460px] xs:min-h-[450px] rounded-[30px] p-[1px] bg-gradient-to-b from-white/[0.22] via-white/[0.08] to-white/[0.03] shadow-[0_30px_70px_-20px_rgba(0,0,0,0.95)] relative overflow-hidden group-hover:from-white/30 transition-all duration-300">
              {/* Inner Matte Obsidian Glass Body */}
              <div className="w-full h-full rounded-[29px] bg-gradient-to-b from-[#14151b]/95 via-[#0e0f13]/98 to-[#07080a] p-5 xs:p-6 sm:p-6.5 relative overflow-hidden flex flex-col justify-between backdrop-blur-2xl">
                {/* Apple Card Iridescent Sheen Glare Overlay */}
                <div
                  className="absolute inset-0 pointer-events-none rounded-[29px] transition-opacity duration-200"
                  style={{
                    background: `radial-gradient(circle 320px at ${glare.x}% ${glare.y}%, rgba(255, 255, 255, ${glare.opacity * 0.7}), rgba(186, 230, 253, ${glare.opacity * 0.35}) 40%, rgba(254, 240, 138, ${glare.opacity * 0.2}) 65%, transparent 80%)`,
                  }}
                />

                {/* Top Row: Clean Apple Smart Chip & Contactless Icon */}
                <div className="flex items-center justify-between relative z-10">
                  {/* Apple Card Style Minimalist Titanium Chip */}
                  <div className="w-10 h-7.5 rounded-lg bg-gradient-to-br from-[#2c2d36] via-[#1e1f26] to-[#121318] border border-white/20 p-1 flex flex-col justify-between shadow-inner">
                    <div className="flex justify-between items-center px-0.5">
                      <span className="w-2 h-0.5 bg-white/30 rounded-full" />
                      <span className="w-2 h-0.5 bg-white/30 rounded-full" />
                    </div>
                    <div className="self-center w-3.5 h-2 rounded-[2px] border border-white/25 flex items-center justify-center bg-white/[0.03]">
                ...
                {/* Center Identity Section: Pristine Avatar & Typography */}
                <div className="my-auto py-2 text-center flex flex-col items-center relative z-10">
                  {/* Clean Circular Portrait with Apple-like Hairline Ring */}
                  <div className="relative w-22 h-22 xs:w-24 xs:h-24 sm:w-26 sm:h-26 rounded-full p-[1.5px] bg-gradient-to-b from-white/30 via-white/10 to-transparent shadow-2xl mb-3">
                    <div className="relative w-full h-full rounded-full bg-[#0a0b0f] overflow-hidden">
                      <Image
                  ...
                  {/* Name in Apple Crisp Typography */}
                  <h2 className="text-2xl xs:text-3xl font-semibold tracking-tight text-white leading-tight">
                    Subhash Ketagoda
                  </h2>

                  <p className="text-white/60 text-xs xs:text-sm font-sans tracking-wide mt-1">
                    Senior Full-Stack Architect
                  </p>

                  <div className="flex items-center gap-1.5 text-white/40 text-[11px] xs:text-xs mt-1">
                    <MapPin className="w-3 h-3 text-white/50 shrink-0" />
                    <span>Colombo, Sri Lanka &bull; Remote</span>
                  </div>

                  {/* Phone Pill */}
                  <div className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.05] hover:bg-white/[0.09] border border-white/10 text-white/90 text-xs font-mono transition-colors">
                ...
                <div className="flex flex-wrap justify-center gap-1.5 relative z-10 mb-2">
                  {["Next.js 15", "React 19", "TypeScript", "POS Engine", "Cloud"].map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-0.5 rounded-full bg-white/[0.03] border border-white/[0.08] text-[10px] font-sans text-white/70"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                {/* Bottom Row: Minimalist Cardholder Stamp & Flip Hint */}
                <div className="pt-3 border-t border-white/[0.08] flex items-center justify-between text-[10px] font-sans text-white/40 relative z-10">
                  <span className="font-mono tracking-widest text-[9px] uppercase text-white/50">
                    SUBHASH KETAGODA
                  </span>
            ...
            {/* =========================================================================
                BACK SIDE: MINIMALIST STRIPE, SIGNATURE, SCANNABLE QR & CHANNELS
               ========================================================================= */}
            <div className="backface-hidden rotate-y-180 absolute inset-0 w-full h-full min-h-[460px] xs:min-h-[450px] rounded-[30px] p-[1px] bg-gradient-to-b from-white/[0.22] via-white/[0.08] to-white/[0.03] shadow-[0_30px_70px_-20px_rgba(0,0,0,0.95)] overflow-hidden group-hover:from-white/30 transition-all duration-300">
              {/* Inner Matte Body */}
              <div className="w-full h-full rounded-[29px] bg-gradient-to-b from-[#14151b]/95 via-[#0e0f13]/98 to-[#07080a] p-5 xs:p-6 relative overflow-hidden flex flex-col justify-between backdrop-blur-2xl">
                {/* Glare */}
                <div
                  className="absolute inset-0 pointer-events-none rounded-[29px] transition-opacity duration-200"
                  style={{
                    background: `radial-gradient(circle 320px at ${100 - glare.x}% ${glare.y}%, rgba(255, 255, 255, ${glare.opacity * 0.7}), transparent 75%)`,
                  }}
                />

                {/* 1. Sleek Matte Black Magnetic Stripe */}
                <div className="-mx-5 xs:-mx-6 -mt-5 xs:-mt-6 h-10 bg-black/90 border-y border-white/[0.08] flex items-center px-5 justify-between">
                  <span className="font-mono text-[8px] tracking-[0.2em] text-white/30 uppercase">
                    AUTHENTIC EXECUTIVE VCARD // NFC
                  </span>
                  <span className="font-mono text-[8px] text-white/40">2026</span>
                </div>

                {/* 2. Signature Panel & Security Code */}
                <div className="my-1.5 flex items-center gap-2">
                  <div className="flex-1 h-8 rounded-lg bg-white/[0.08] border border-white/10 px-3 flex items-center justify-between">
                    <img
                      src="/images/subhash-signature.png"
                      alt="Subhash Ketagoda Signature"
                      className="h-6 w-auto object-contain filter invert contrast-125 opacity-80"
                    />
                    <span className="font-mono text-[8px] text-white/40 uppercase">
                      SIGNATURE
                    </span>
                  </div>
                  <div className="h-8 px-2.5 rounded-lg bg-white/[0.05] border border-white/10 flex items-center justify-center">
                    <span className="font-mono text-[10px] text-white/70">CVV 969</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 my-auto relative z-10">
                  ...
                  <div
                    ...
                    className="w-[105px] xs:w-[115px] p-2.5 rounded-2xl bg-white flex flex-col items-center justify-center shrink-0 shadow-2xl transition-transform active:scale-95 cursor-zoom-in"
                  ...
                  >
                    ...
                    <span className="text-[8px] font-sans font-medium text-black/70 mt-1 uppercase tracking-wider text-center">
                      Scan to Add
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    ...
                    <a
                      ...
                      className="w-full px-3 py-1.5 rounded-xl bg-white/[0.05] hover:bg-white/[0.1] border border-white/10 flex items-center justify-between text-left text-white transition-all active:scale-95"
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <MessageCircle className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                        <span className="truncate text-xs font-medium">WhatsApp</span>
                      </div>
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="truncate text-xs font-mono">+94 78 965 6969</span>
                      </div>
                      <button
                        type="button"
                ...
                <div className="flex items-center justify-around py-1.5 border-t border-white/[0.08] relative z-10">
                  ...
                  <a
                    href="tel:+94789656969"
                    onClick={(e) => e.stopPropagation()}
                    className="p-2 rounded-full hover:bg-white/10 text-sky-400 transition-colors"
                    aria-label="Call"
                  >
                ...
                {/* Bottom Row */}
                <div className="pt-2 border-t border-white/[0.08] flex items-center justify-between text-[10px] font-sans text-white/40 relative z-10">
                  <div className="flex items-center gap-1 text-white/60">
                    <RotateCw className="w-3 h-3 animate-reverse-spin" />
                    <span>Tap to flip front</span>
                  </div>
                  <span className="font-mono text-[9px] text-white/40">APPLE PASS FORMAT</span>
                </div>
              </div>
            </div>
          </div>
